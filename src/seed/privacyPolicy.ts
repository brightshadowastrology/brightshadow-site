...
export async function seedPrivacyPolicy(payload: Payload) {
  ...
  const maintainingConfidentialityBlock = contentBlock(
    richText([
  ...
  const consultationsBlock = contentBlock(
    richText([
  ...
  const dataCollectionsBlock = contentBlock(
    richText([
  ...
  await payload.create({
    ...
    data: {
      ...
      layout: [
        introBlock,
        maintainingConfidentialityBlock,
        consultationsBlock,
        dataCollectionsBlock,
      ],
      _status: "published",
    },
  });

  console.log("Privacy Policy page seeded with 4 ContentBlocks.");
}

...
export async function seedTermsAndConditions(payload: Payload) {
  ...
  const termsAndConditionsBlock = {
    ...
    columns: [
      column(richText([lexicalHeading("Terms and Conditions", "h1")])),
      column(
        richText([
  ...
  await payload.create({
    ...
    data: {
      ...
      layout: [termsAndConditionsBlock],
      _status: "published",
    },
  });

  console.log(
    "Terms & Conditions page seeded with 1 ContentBlock (7 columns + 6 dividers).",
  );
}
